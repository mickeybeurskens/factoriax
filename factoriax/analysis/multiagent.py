...
def plot_comparative_raster(
    ...
    player_labels : list[str], optional
...
def role_divergence(
    ...
    jsd_matrix : np.ndarray
        Shape ``(P, P)`` symmetric matrix of JSD values in [0, 1].
    """
    _require_multi_player(traj)
    P = traj.num_players
...
def plot_role_divergence(
    ...
    ----------
    traj : Trajectory
    phases : list of (start, end), optional
        If provided, creates subplots for each phase.
    player_labels : list[str], optional

    Returns
    -------
    fig, ax_or_axes : Figure, Axes or ndarray of Axes
    """
    _require_multi_player(traj)
    P = traj.num_players
...
def spatial_overlap(
    ...
    -------
    overlap : np.ndarray
        Shape ``(T,)`` — fraction of episodes where all players are
        within *distance_threshold* of each other at each timestep.
    """
    _require_multi_player(traj)
...
def joint_action_matrix(
    ...
    -------
    mat : np.ndarray
        Shape ``(num_actions, num_actions)``.  ``mat[i, j]`` is the
        frequency of player_a taking action i while player_b takes action j.
    """
    _require_multi_player(traj)
