...
# Human-readable display names for each MachineType, derived from the
# enum: underscores become spaces and each word is title-cased, giving
# e.g. "Conveyor Belt". Every member is covered, so a new MachineType
...
MACHINE_TYPE_NAMES: dict[int, str] = {
    int(mt): mt.name.replace("_", " ").title() for mt in MachineType
}
