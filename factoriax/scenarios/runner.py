"""ScenarioRunner: executes policies against a Scenario.

The runner owns all simulation concerns — environment stepping, observation
extraction, PRNG management, trajectory collection. A ``Scenario`` tells
the runner what levels to run and how to score results; the runner tells
nobody anything, it just executes.

This separation means scenarios can be defined, tested, and reasoned about
...
class ScenarioRunner:
    """Executes a list of policies against a ``Scenario`` and returns results.
    ...
    For multi-agent scenarios: each tick, all players act in sequence and
    ...
    def _resolve_blocked(
        self, scenario: Scenario, scenario_level: ScenarioLevel
    ) -> frozenset[int]:
        """Pick the effective ``blocked_actions`` mask for *scenario_level*.

        Per-level ``ScenarioLevel.blocked_actions`` wins when set —
        including when set to an empty ``frozenset()``, which means
        "this level explicitly has no mask". When the per-level field
        is ``None``, fall back to the scenario's class-level
        ``blocked_actions`` attribute (used by ``RocketScenario`` to
        share a single mask across all its levels).

        Args:
            scenario: Scenario being run.
            scenario_level: Level whose mask we're resolving.

        Returns:
            Frozen set of action ids to block. Empty set means no mask.
        """
        if scenario_level.blocked_actions is not None:
            return frozenset(scenario_level.blocked_actions)
        return frozenset(getattr(scenario, "blocked_actions", ()) or ())
    ...
    def _ensure_env(
        ...
        scenario-advertised) and the per-level ``blocked_actions``
        ...
        Args:
            scenario: Scenario being run.
            scenario_level: Level about to execute.

        Returns:
            Resolved achievement function, or ``None`` if neither side
            advertises one.
        """
        resolved = self._achievement_fn or getattr(scenario, "achievement_fn", None)
        blocked = self._resolve_blocked(scenario, scenario_level)
    ...
    def _achievements(self, state: EnvState) -> np.ndarray | None:
        ...
        "this scenario tracks achievements" from "no tracking attempted".
        """
    ...
    def run(
        ...
        """Run policies through all scenario levels and return aggregated results.
        ...
        Returns:
            ``ScenarioResult`` containing per-level results and the
            aggregate score computed by ``scenario.score``.
        ...
        if len(policies) != scenario.num_players:
            noun = "policy" if scenario.num_players == 1 else "policies"
            raise ValueError(
                f"Scenario '{scenario.name}' requires {scenario.num_players} "
        ...
        for scenario_level in scenario.levels():
            self._ensure_env(scenario, scenario_level)
        ...
        aggregate = scenario.score(level_results)
        logger.info(
            "Scenario '%s' complete: aggregate_score=%.3f",
            scenario.name,
            aggregate,
        )
    ...
    def run_batched(
        ...
        Only supports single-player scenarios. The policy must be a
        ...
        Returns:
            List of ``ScenarioResult``, one per seed, in the same
            order as ``seeds``.

        Raises:
            ValueError: If the scenario requires more than one player.
        """
        if scenario.num_players != 1:
            raise ValueError(
                "run_batched only supports single-player scenarios, "
                f"got num_players={scenario.num_players}."
            )
        _obs_fn = obs_fn if obs_fn is not None else global_array
        num_seeds = len(seeds)
        ...
        level_data: list[tuple[ScenarioLevel, EnvState, np.ndarray, np.ndarray]] = []

        for scenario_level in levels:
            self._ensure_env(scenario, scenario_level)
            params = scenario_level.env_params
            state0 = build_state(scenario_level.level, params)
            ...
            level_data.append(
                (scenario_level, final_states, all_actions_np, timesteps_np)
            )

        # Assemble per-seed ScenarioResults.
        ...
        for i in range(num_seeds):
            ...
            for (
                ...
                score = scenario.score_level(scenario_level, items_mined)
            ...
            agg = scenario.score(level_results)
            results.append(
                ScenarioResult(
                    scenario_name=scenario.name,
                    level_results=level_results,
        ...
        logger.info(
            "Scenario '%s' batched (%d seeds): mean=%.3f  std=%.3f",
            scenario.name,
            num_seeds,
    ...
    def _run_level(
        ...
        The level state is built deterministically from ``scenario_level.level``
        and ``scenario_level.env_params`` via ``build_state`` — no random key
        ...
        scenario's ``achievement_fn`` attribute), the latched unlock
        mask is read directly off ``state.achievements_unlocked``
        and returned on the result.

        Args:
            scenario: Scenario owning this level (provides per-level scoring).
            scenario_level: Level to run.
