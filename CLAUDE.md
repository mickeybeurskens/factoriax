...
- Direct but explanatory. Take time to paint the full picture, don't just state facts.
- Flowing prose. Let sentences build on each other naturally. Skip headers if the content flows without them.
- Anti-hype. No superlatives or marketing speak. The work speaks for itself.
- Technically confident. Assume the reader is smart but give enough context to understand the "why".
- Concrete benefits. Explain practical value in real terms (e.g. "more training runs" not "faster").
- Natural punctuation. Commas and periods over dashes and semicolons.
- A bit of personality. It should be enjoyable to read. Dry is boring.

## Project Structure

- Source code lives in `factoriax/`
- Uses JAX for GPU-accelerated simulation
