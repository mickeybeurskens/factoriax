"""Regression tests for reward functions that read from pouch inventory.

These tests verify that reward functions correctly read from
player_inventory and machine_inventory after the slot-to-pouch migration.
"""
...
class TestMinerOutputReward:
    """Tests for miner_output_reward reading machine_inventory."""
    ...
    def test_positive_when_miner_produces_ore(
        ...
        """Ore increase in a miner's machine_inventory produces reward."""
        mt = jnp.full((4, 4), int(MachineType.MINER), dtype=jnp.int32)
        inv_before = jnp.zeros(
            (4, 4, NUM_ITEM_TYPES), dtype=MACHINE_INVENTORY_COUNT_DTYPE,
        )
        prev = state_factory(
            world_map=jnp.zeros((4, 4), dtype=jnp.int32),
            machine_types=mt,
            machine_inventory=inv_before,
        )
        inv_after = jnp.zeros(
            (4, 4, NUM_ITEM_TYPES), dtype=MACHINE_INVENTORY_COUNT_DTYPE,
        )
        inv_after = inv_after.at[0, 0, int(ItemType.COAL)].set(3)
        new = state_factory(
            world_map=jnp.zeros((4, 4), dtype=jnp.int32),
            machine_types=mt,
